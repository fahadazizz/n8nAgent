...
def run_simple_test():
    """Test without simulator for faster execution."""
    print("Building Graph...")
    graph = build_graph()
    
    user_request = "Create a webhook that posts to Slack"
    
    print(f"Invoking graph with request: {user_request}")
    initial_state = {
        "user_request": user_request, 
        "messages": [],
        "validation_status": "verified"  # Skip simulation for speed
    }
    ...
    try:
        ...
        if final_state.get("intent"):
            print("\n[Intent]:", final_state["intent"].get("raw_intent"))
